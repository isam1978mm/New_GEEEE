...
AI_BEH_ALLOY_STATUE_RECOVERY_SCHEMA_VERSION = (
    "ai_beh_alloy_statue_recovery_v1"
)
AI_BEH_ALLOY_STATUE_RECOVERY_REPORT_RELATIVE_PATH = (
    "manifests/ai_beh_alloy_statue_recovery_report.json"
)
...
class AIBehAlloyStatueRecoveryItem:
    id: str
    notebook_output: str
    family: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.target_mode == "public_shared":
            raise ValueError("Phase 4H10 recovery items must not target public_shared")
        if self.http_servable:
            raise ValueError(
                "Phase 4H10 recovery items must not default http_servable true"
            )
        if self.runtime_output_verified:
            raise ValueError(
                "Phase 4H10 recovery does not verify runtime output presence"
            )
        if self.notebook_value_parity_verified:
            raise ValueError(
                "Phase 4H10 recovery does not verify notebook value parity"
            )
...
_ITEM = AIBehAlloyStatueRecoveryItem(
    id="ai_beh_alloys_statues",
    notebook_output="AI_BEH_Alloys_Statues_REL_ND_DOM_lin_640.tif",
    family="AI_BEH semantic rasters",
    current_app_status=(
        "missing; notebook builder cells are visible, but the app has no standalone "
        "writer for this notebook-named AI_BEH semantic raster"
    ),
    source_status="exact_source_found",
    authoritative_source_available=True,
    source_reference=(
        "notebooks/new.ipynb lines around 24307-24310 keep the exact builder cell: "
        "s2_col.normalizedDifference(['B4', 'B8']) is renamed "
        "AI_BEH_Alloys_Statues_REL_ND_DOM_lin_640. Later notebook cells around 24372 "
        "and 35479 keep the exported filename visible in signature and candidate-file "
        "tables."
    ),
    ...
    expected_units=(
        "Notebook formula produces a semantic raster from Sentinel-2 reflectance bands. "
        "Frozen notebook references are still required to lock final unit wording and "
        "numeric tolerance."
    ),
    expected_nodata_policy=(
        "The notebook source keeps the formula and output name visible, but the exact "
        "exported nodata or NaN persistence remains unresolved until frozen notebook "
        "references are captured."
    ),
    required_reference_outputs=("AI_BEH_Alloys_Statues_REL_ND_DOM_lin_640.tif",),
    ...
    recommended_next_action=(
        "Capture the frozen Alloys/Statues raster and run the Phase 4H10 verifier "
...
def get_ai_beh_alloy_statue_recovery_checklist() -> tuple[AIBehAlloyStatueRecoveryItem]:
    """Return the Phase 4H10 recovery checklist for the alloy/statue raster."""

    return (_ITEM,)


def write_ai_beh_alloy_statue_recovery_report(
    run_dir: str | Path,
    run_id: str,
    *,
    item: AIBehAlloyStatueRecoveryItem = _ITEM,
    report_relative_path: str | Path = AI_BEH_ALLOY_STATUE_RECOVERY_REPORT_RELATIVE_PATH,
) -> Path:
    ...
    payload = {
        "schema_version": AI_BEH_ALLOY_STATUE_RECOVERY_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "items": [item.to_dict()],
        "counts_by_source_status": _counts_by("source_status", item),
        "counts_by_implementation_status": _counts_by(
            "implementation_status", item
        ),
        "phase_4h10_formula_changes": False,
        "notes": (
            "Phase 4H10 is recovery and verification-contract work only. It does not "
            "implement the AI_BEH alloy/statue raster, change semantic formulas, or "
...
def _counts_by(
    field_name: str,
    item: AIBehAlloyStatueRecoveryItem,
) -> dict[str, int]:
