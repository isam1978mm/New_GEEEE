...
def verify_ai_beh_alloy_statue_parity(
    ...
    *,
    ...
    output_item = _verify_output(
        app_root / AI_BEH_ALLOY_STATUE_OUTPUT_NAME,
        reference_root / AI_BEH_ALLOY_STATUE_OUTPUT_NAME,
        rasterio_available=rasterio_available,
        atol=atol,
        rtol=rtol,
    )
    overall_status = _overall_status(output_item["status"])
    payload = {
...
def _verify_output(
    ...
    *,
    ...
    item = _base_output_item(app_path, reference_path)
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App AI_BEH alloy/statue output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook AI_BEH alloy/statue reference is missing.",
        )
...
def _base_output_item(app_path: Path, reference_path: Path) -> dict[str, Any]:
    return {
        "output_name": AI_BEH_ALLOY_STATUE_OUTPUT_NAME,
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported AI_BEH alloy/statue output status: {status}")
    item["status"] = status
...
def _overall_status(status: str) -> str:
    if status == "passed":
        overall_status = "passed"
    elif status == "comparison_unavailable":
        overall_status = "comparison_unavailable"
    elif status in {"missing_app_output", "missing_reference_output"}:
    ...
    if overall_status not in ALLOWED_OVERALL_STATUSES:
        raise ValueError(
            f"unsupported AI_BEH alloy/statue overall status: {overall_status}"
        )
