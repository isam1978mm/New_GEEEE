...
AI_BEH_EXTENDED_VERIFICATION_SCHEMA_VERSION = (
    "ai_beh_extended_parity_verification_v1"
)
AI_BEH_EXTENDED_REPORT_RELATIVE_PATH = (
    "manifests/ai_beh_extended_parity_verification.json"
)
AI_BEH_EXTENDED_CLASSIFICATION = "notebook-parity semantic raster stage"
AI_BEH_EXTENDED_FAMILY = "AI_BEH semantic rasters"
...
class AIBehExtendedVerificationResult:
    report_path: Path
    overall_status: str
...
def verify_ai_beh_extended_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = AI_BEH_EXTENDED_REPORT_RELATIVE_PATH,
) -> AIBehExtendedVerificationResult:
    """Verify AI_BEH extended parity against frozen notebook reference rasters."""
    ...
    output_items = tuple(
        ...
        for output_name in AI_BEH_EXTENDED_OUTPUT_NAMES
    )
    overall_status = _overall_status(output_items)
    payload = {
        "schema_version": AI_BEH_EXTENDED_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "outputs": list(output_items),
        "classification": AI_BEH_EXTENDED_CLASSIFICATION,
        "family": AI_BEH_EXTENDED_FAMILY,
    ...
    return AIBehExtendedVerificationResult(
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App AI_BEH extended output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook AI_BEH extended reference is missing.",
        )
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported AI_BEH extended output status: {status}")
    item["status"] = status
...
def _overall_status(outputs: tuple[dict[str, Any], ...]) -> str:
    ...
    if overall_status not in ALLOWED_OVERALL_STATUSES:
        raise ValueError(f"unsupported AI_BEH extended overall status: {overall_status}")
