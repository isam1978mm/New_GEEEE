...
AI_BEH_LOGIC_RECOVERY_SCHEMA_VERSION = "ai_beh_logic_recovery_v1"
AI_BEH_LOGIC_RECOVERY_REPORT_RELATIVE_PATH = (
    "manifests/ai_beh_logic_recovery_report.json"
)
...
class AIBehLogicRecoveryItem:
    ...
    def __post_init__(self) -> None:
        ...
        if self.target_mode == "public_shared":
            raise ValueError("Phase 4H7 recovery items must not target public_shared")
        if self.http_servable:
            raise ValueError(
                "Phase 4H7 recovery items must not default http_servable true"
            )
        if self.runtime_output_verified:
            raise ValueError("Phase 4H7 recovery does not verify runtime output presence")
        if self.notebook_value_parity_verified:
            raise ValueError("Phase 4H7 recovery does not verify notebook value parity")

    def to_dict(self) -> dict[str, object]:
        return asdict(self)


_CHECKLIST: tuple[AIBehLogicRecoveryItem, ...] = (
    AIBehLogicRecoveryItem(
        id="ai_beh_secretentry_logic",
        notebook_output="AI_BEH_SecretEntry_REL_ND_DOM_lin_640.tif",
        ...
        source_reference=(
            "notebooks/new.ipynb lines around 24207-24209 keep the exact builder cell: "
            "s2.normalizedDifference(['B12', 'B8A']) is renamed "
            "AI_BEH_SecretEntry_REL_ND_DOM_lin_640. Later notebook cells around 35474 "
            "keep the exported filename visible in the candidate file table."
        ),
        expected_input_outputs=_COMMON_INPUTS,
        expected_formula_summary=(
            "AI_BEH_SecretEntry_REL_ND_DOM_lin_640 = normalizedDifference(B12, B8A)."
        ),
        expected_dtype="unknown",
        expected_units=_COMMON_UNITS,
        expected_nodata_policy=_COMMON_NODATA,
        required_reference_outputs=("AI_BEH_SecretEntry_REL_ND_DOM_lin_640.tif",),
        ...
        blocker=(
            "The builder cell and notebook export name are visible, but frozen notebook "
            "references are still required to lock final metadata and numeric tolerance."
        ),
        recommended_next_action=(
            "Capture the frozen SecretEntry raster and run the Phase 4H7 verifier before "
    ...
    AIBehLogicRecoveryItem(
        id="ai_beh_statuelogic_logic",
        notebook_output="AI_BEH_StatueLogic_REL_Diff_DOM_lin_640.tif",
        family="AI_BEH semantic rasters",
        current_app_status=_COMMON_CURRENT_STATUS,
        source_status="exact_source_found",
        authoritative_source_available=True,
        source_reference=(
            "notebooks/new.ipynb lines around 24211-24213 keep the exact builder cell: "
            "s2.select('B11').subtract(s2.select('B4')) is renamed "
            "AI_BEH_StatueLogic_REL_Diff_DOM_lin_640. The current evidence keeps the "
            "builder formula visible, but a later notebook candidate-file table entry is "
            "not yet locked separately from the builder cell."
        ),
        expected_input_outputs=_COMMON_INPUTS,
        expected_formula_summary=(
            "AI_BEH_StatueLogic_REL_Diff_DOM_lin_640 = B11 - B4."
        ),
        expected_dtype="unknown",
        expected_units=_COMMON_UNITS,
        expected_nodata_policy=_COMMON_NODATA,
        required_reference_outputs=("AI_BEH_StatueLogic_REL_Diff_DOM_lin_640.tif",),
        required_metadata=_COMMON_REQUIRED_METADATA,
        target_mode="notebook_parity",
        classification="notebook-parity semantic raster stage",
        http_servable=False,
        ...
        implementation_status="requires_reference_output",
        blocker=(
            "The builder cell is visible, but frozen notebook references are still "
            "required to lock final metadata and numeric tolerance."
        ),
        recommended_next_action=(
            "Capture the frozen StatueLogic raster and run the Phase 4H7 verifier before "
            "any implementation slice."
        ),
        notes=(
            "This is a standalone notebook output and a downstream stack component. "
            "Current evidence supports the formula lock without asserting a later export "
            "table entry beyond the builder cell."
        ),
    ),
)


def get_ai_beh_logic_recovery_checklist() -> tuple[AIBehLogicRecoveryItem, ...]:
    """Return the Phase 4H7 recovery checklist for the AI_BEH logic rasters."""

    return _CHECKLIST


def write_ai_beh_logic_recovery_report(
    run_dir: str | Path,
    run_id: str,
    *,
    items: Iterable[AIBehLogicRecoveryItem] | None = None,
    report_relative_path: str | Path = AI_BEH_LOGIC_RECOVERY_REPORT_RELATIVE_PATH,
) -> Path:
    ...
    payload = {
        "schema_version": AI_BEH_LOGIC_RECOVERY_SCHEMA_VERSION,
        "run_id": run_id,
        ...
        "phase_4h7_formula_changes": False,
        "notes": (
            "Phase 4H7 is recovery and verification-contract work only. It does not "
            "implement AI_BEH logic rasters, change semantic formulas, or introduce "
            "public serving."
        ),
    }
...
def _counts_by(
    field_name: str,
    items: Iterable[AIBehLogicRecoveryItem],
) -> dict[str, int]:
