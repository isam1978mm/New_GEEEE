...
def verify_ai_beh_logic_parity(
    ...
    *,
    ...
    output_items = tuple(
        ...
        for output_name in AI_BEH_LOGIC_OUTPUT_NAMES
    )
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App AI_BEH logic output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook AI_BEH logic reference is missing.",
        )
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported AI_BEH logic output status: {status}")
    item["status"] = status
...
def _overall_status(outputs: tuple[dict[str, Any], ...]) -> str:
    ...
    if overall_status not in ALLOWED_OVERALL_STATUSES:
        raise ValueError(f"unsupported AI_BEH logic overall status: {overall_status}")
