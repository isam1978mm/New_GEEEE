...
AI_READY_ANOMALY_VERIFICATION_SCHEMA_VERSION = (
    "ai_ready_anomaly_parity_verification_v1"
)
AI_READY_ANOMALY_REPORT_RELATIVE_PATH = (
    "manifests/ai_ready_anomaly_parity_verification.json"
)
AI_READY_ANOMALY_CLASSIFICATION = "notebook-parity semantic raster stage"
AI_READY_ANOMALY_FAMILY = "AI_READY semantic rasters"
...
class AIReadyAnomalyVerificationResult:
    report_path: Path
    overall_status: str
...
def verify_ai_ready_anomaly_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = AI_READY_ANOMALY_REPORT_RELATIVE_PATH,
) -> AIReadyAnomalyVerificationResult:
    """Verify AI_READY anomaly rasters against frozen notebook references."""
    ...
    output_items = tuple(
        ...
        for output_name in AI_READY_ANOMALY_OUTPUT_NAMES
    )
    overall_status = _overall_status(output_items)
    payload = {
        "schema_version": AI_READY_ANOMALY_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "outputs": list(output_items),
        "classification": AI_READY_ANOMALY_CLASSIFICATION,
        "family": AI_READY_ANOMALY_FAMILY,
    ...
    return AIReadyAnomalyVerificationResult(
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App anomaly output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook anomaly reference is missing.",
        )
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported AI_READY anomaly output status: {status}")
    item["status"] = status
...
def _overall_status(outputs: tuple[dict[str, Any], ...]) -> str:
    ...
    if overall_status not in ALLOWED_OVERALL_STATUSES:
        raise ValueError(
            f"unsupported AI_READY anomaly overall status: {overall_status}"
        )
