...
AI_READY_FRACTION_VERIFICATION_SCHEMA_VERSION = (
    "ai_ready_fraction_parity_verification_v1"
)
AI_READY_FRACTION_REPORT_RELATIVE_PATH = (
    "manifests/ai_ready_fraction_parity_verification.json"
)
AI_READY_FRACTION_CLASSIFICATION = "notebook-parity semantic raster stage"
AI_READY_FRACTION_FAMILY = "AI_READY semantic rasters"
...
class AIReadyFractionVerificationResult:
    report_path: Path
    overall_status: str
...
def verify_ai_ready_fraction_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = AI_READY_FRACTION_REPORT_RELATIVE_PATH,
) -> AIReadyFractionVerificationResult:
    """Verify Fraction_* parity against frozen notebook reference rasters."""
    ...
    output_items = tuple(
        ...
        for output_name in AI_READY_FRACTION_OUTPUT_NAMES
    )
    overall_status = _overall_status(output_items)
    payload = {
        "schema_version": AI_READY_FRACTION_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "outputs": list(output_items),
        "classification": AI_READY_FRACTION_CLASSIFICATION,
        "family": AI_READY_FRACTION_FAMILY,
    ...
    return AIReadyFractionVerificationResult(
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App Fraction output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook Fraction reference is missing.",
        )
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported AI_READY Fraction output status: {status}")
    item["status"] = status
...
def _overall_status(outputs: tuple[dict[str, Any], ...]) -> str:
    ...
    if overall_status not in ALLOWED_OVERALL_STATUSES:
        raise ValueError(f"unsupported AI_READY Fraction overall status: {overall_status}")
