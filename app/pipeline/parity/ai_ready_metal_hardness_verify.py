...
AI_READY_METAL_HARDNESS_VERIFICATION_SCHEMA_VERSION = (
    "ai_ready_metal_hardness_parity_verification_v1"
)
AI_READY_METAL_HARDNESS_REPORT_RELATIVE_PATH = (
    "manifests/ai_ready_metal_hardness_parity_verification.json"
)
AI_READY_METAL_HARDNESS_CLASSIFICATION = "notebook-parity semantic raster stage"
AI_READY_METAL_HARDNESS_FAMILY = "AI_READY semantic rasters"
AI_READY_METAL_HARDNESS_OUTPUT_NAME = "AI_READY_640_Metal_Hardness.tif"
...
class AIReadyMetalHardnessVerificationResult:
    report_path: Path
    overall_status: str
...
def verify_ai_ready_metal_hardness_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = AI_READY_METAL_HARDNESS_REPORT_RELATIVE_PATH,
) -> AIReadyMetalHardnessVerificationResult:
    """Verify Metal Hardness parity against a frozen notebook reference raster."""
    ...
    output_items = (
        _verify_one_output(
            ...
            app_root / AI_READY_METAL_HARDNESS_OUTPUT_NAME,
            reference_root / AI_READY_METAL_HARDNESS_OUTPUT_NAME,
    ...
    payload = {
        "schema_version": AI_READY_METAL_HARDNESS_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "outputs": list(output_items),
        "classification": AI_READY_METAL_HARDNESS_CLASSIFICATION,
        "family": AI_READY_METAL_HARDNESS_FAMILY,
    ...
    return AIReadyMetalHardnessVerificationResult(
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App Metal Hardness output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook Metal Hardness reference is missing.",
        )
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported AI_READY Metal Hardness output status: {status}")
    item["status"] = status
...
def _overall_status(outputs: tuple[dict[str, Any], ...]) -> str:
    ...
    if overall_status not in ALLOWED_OVERALL_STATUSES:
        raise ValueError(
            f"unsupported AI_READY Metal Hardness overall status: {overall_status}"
        )
