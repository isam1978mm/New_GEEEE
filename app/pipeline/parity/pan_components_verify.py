...
PAN_COMPONENTS_VERIFICATION_SCHEMA_VERSION = "pan_components_parity_verification_v1"
PAN_COMPONENTS_REPORT_RELATIVE_PATH = "manifests/pan_components_parity_verification.json"
PAN_COMPONENTS_CLASSIFICATION = "notebook-parity"
PAN_COMPONENTS_FAMILY = "panchromatic/optical outputs"
...
class PanComponentsVerificationResult:
    report_path: Path
    overall_status: str
...
def verify_pan_components_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = PAN_COMPONENTS_REPORT_RELATIVE_PATH,
) -> PanComponentsVerificationResult:
    """Verify PAN component files against frozen notebook references."""
    ...
    output_items = tuple(
        ...
        for output_name in PAN_COMPONENT_OUTPUT_NAMES
    )
    ...
    payload = {
        "schema_version": PAN_COMPONENTS_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "outputs": list(output_items),
        "classification": PAN_COMPONENTS_CLASSIFICATION,
        "family": PAN_COMPONENTS_FAMILY,
    ...
    return PanComponentsVerificationResult(
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App PAN component output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook PAN component reference is missing.",
        )
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported PAN component output status: {status}")
    item["status"] = status
    ...
    *,
    ...
    if item["within_tolerance"]:
        return _finish_output(
            ...
            notes="App and notebook PAN component NPY outputs match within tolerance.",
        )
    return _finish_output(
        item,
    ...
    *,
    ...
    if item["within_tolerance"]:
        return _finish_output(
            ...
            runtime_output_verified=True,
            notebook_value_parity_verified=True,
            notes="App and notebook PAN component raster outputs match within tolerance.",
        )
    return _finish_output(
        item,
        status="value_mismatch",
...
def _find_expected_file(root: Path, output_name: str) -> Path:
    ...
    if direct.is_file():
        return direct
    nested = root / "OPT" / ("PAN_NPY_640" if output_name.endswith(".npy") else "PAN_TIFS_640") / output_name
    if nested.is_file():
        return nested
    return direct
