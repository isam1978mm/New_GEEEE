...
PAN_STACK_VERIFICATION_SCHEMA_VERSION = "pan_layers_stack_verification_v1"
PAN_STACK_REPORT_RELATIVE_PATH = "manifests/pan_layers_stack_verification.json"
PAN_STACK_OUTPUT_NAME = "PAN_LAYERS_STACK_640.npy"
PAN_STACK_FAMILY = "panchromatic/optical outputs"
PAN_STACK_CLASSIFICATION = "notebook-parity"
...
class PanStackVerificationResult:
    report_path: Path
    overall_status: str
    status: str


def verify_pan_stack_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = PAN_STACK_REPORT_RELATIVE_PATH,
) -> PanStackVerificationResult:
    """Verify a future app PAN stack against a notebook reference."""
    ...
    if not app_exists:
        _finish_report(
            ...
            notes="App PAN_LAYERS_STACK_640.npy output is missing.",
        )
        ...
        _finish_report(
            ...
            notes="Frozen notebook PAN_LAYERS_STACK_640.npy reference is missing.",
        )
    else:
        try:
    ...
    return PanStackVerificationResult(
...
def _base_report(
    *,
    ...
    return {
        "schema_version": PAN_STACK_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "output_name": PAN_STACK_OUTPUT_NAME,
        ...
        "family": PAN_STACK_FAMILY,
        "classification": PAN_STACK_CLASSIFICATION,
    ...
    *,
    ...
    if payload["within_tolerance"]:
        _finish_report(
            ...
            notes="App and notebook PAN stack outputs match within tolerance.",
        )
        return
...
def _finish_report(
    ...
    *,
    ...
    if status not in ALLOWED_STATUSES:
        raise ValueError(f"unsupported PAN stack status: {status}")
    if overall_status not in ALLOWED_OVERALL_STATUSES:
        raise ValueError(f"unsupported PAN stack overall_status: {overall_status}")
...
def _find_expected_file(root: Path) -> Path:
    direct = root / PAN_STACK_OUTPUT_NAME
    if direct.is_file():
        return direct
    nested = root / "NPY_STACKS" / PAN_STACK_OUTPUT_NAME
    if nested.is_file():
        return nested
    return direct
