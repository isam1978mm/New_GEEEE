...
SAR_ASC_DESC_VERIFICATION_SCHEMA_VERSION = "sar_asc_desc_support_stack_verification_v1"
SAR_ASC_DESC_REPORT_RELATIVE_PATH = (
    "manifests/sar_asc_desc_support_stack_verification.json"
)
SAR_ASC_DESC_CLASSIFICATION = "notebook-parity"
SAR_ASC_DESC_FAMILY = "SAR/radar outputs"
...
class SarAscDescVerificationResult:
    report_path: Path
    overall_status: str
...
def verify_sar_asc_desc_support_stack_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = SAR_ASC_DESC_REPORT_RELATIVE_PATH,
) -> SarAscDescVerificationResult:
    """Verify ASC/DESC Sentinel-1 support stack files against notebook references.

    This Phase 4E2 helper writes only a JSON verification report under the run
    manifests directory. It does not generate rasters, create aliases, call
    Earth Engine, or integrate with the live pipeline.
    """
    ...
    output_items = tuple(
        ...
        for output_name in SAR_ASC_DESC_OUTPUT_NAMES
    )
    ...
    payload = {
        "schema_version": SAR_ASC_DESC_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "outputs": list(output_items),
        "classification": SAR_ASC_DESC_CLASSIFICATION,
        "family": SAR_ASC_DESC_FAMILY,
    ...
    return SarAscDescVerificationResult(
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App ASC/DESC Sentinel-1 support output is missing.",
        )
    if not reference_exists:
        return _finish_output(
            ...
            notes="Frozen notebook ASC/DESC Sentinel-1 support reference is missing.",
        )
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported SAR ASC/DESC output status: {status}")
    item["status"] = status
    ...
    *,
    ...
    if item["within_tolerance"]:
        return _finish_output(
            ...
            notes="App and notebook ASC/DESC NPY outputs match within tolerance.",
        )
    return _finish_output(
        item,
    ...
    *,
    ...
    if item["within_tolerance"]:
        return _finish_output(
            ...
            runtime_output_verified=True,
            notebook_value_parity_verified=True,
            notes="App and notebook ASC/DESC raster outputs match within tolerance.",
        )
    return _finish_output(
        item,
        status="value_mismatch",
...
def _find_expected_file(root: Path, output_name: str) -> Path:
    ...
    if direct.is_file():
        return direct
    subdir = "NPY_RADAR_BANDS" if output_name.endswith(".npy") else "GEOTIFF_RADAR_BANDS"
    nested = root / subdir / output_name
    if nested.is_file():
        return nested
    return direct
