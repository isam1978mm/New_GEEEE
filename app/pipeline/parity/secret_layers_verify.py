...
SECRET_LAYERS_VERIFICATION_SCHEMA_VERSION = "secret_layers_parity_verification_v1"
SECRET_LAYERS_CLASSIFICATION = "notebook-parity semantic raster stage"
SECRET_LAYERS_REPORT_RELATIVE_PATH = "manifests/secret_layers_parity_verification.json"
...
class SecretLayersVerificationResult:
    report_path: Path
    overall_status: str
...
def verify_secret_layers_parity(
    ...
    *,
    atol: float = 1e-6,
    rtol: float = 1e-6,
    report_relative_path: str | Path = SECRET_LAYERS_REPORT_RELATIVE_PATH,
) -> SecretLayersVerificationResult:
    """Verify secret-layer rasters against a frozen notebook reference tree.
    ...
    output_items = tuple(
        ...
        for output_name in SECRET_LAYERS_OUTPUT_NAMES
    )
    overall_status = _overall_status(output_items)
    payload = {
        "schema_version": SECRET_LAYERS_VERIFICATION_SCHEMA_VERSION,
        "run_id": run_id,
        "created_at": datetime.now(UTC).isoformat(),
        "app_output_dir": str(app_root),
        "notebook_reference_dir": str(reference_root),
        "outputs": list(output_items),
        "classification": SECRET_LAYERS_CLASSIFICATION,
    ...
    return SecretLayersVerificationResult(
    ...
    *,
    ...
    if not app_exists:
        return _finish_output(
            ...
            notes="App secret-layer output is missing.",
        )
    ...
    _record_hashes(item, app_path, reference_path)
...
def _finish_output(
    ...
    *,
    ...
    if status not in ALLOWED_OUTPUT_STATUSES:
        raise ValueError(f"unsupported secret-layer output status: {status}")
    item["status"] = status
...
def _record_hashes(item: dict[str, Any], app_path: Path, reference_path: Path) -> None:
    app_hash = _sha256_file(app_path)
    reference_hash = _sha256_file(reference_path)
    item["app_sha256"] = app_hash
    item["reference_sha256"] = reference_hash
    item["hash_match"] = app_hash == reference_hash
