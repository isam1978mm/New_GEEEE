...
AI_BEH_EXTENDED_OUTPUT_NAMES: tuple[str, str, str] = (
...
AI_BEH_EXTENDED_NPY_OUTPUT_NAMES: dict[str, str] = {
    "AI_BEH_GoldAlloy_REL_Ratio_DOM_lin_640.tif": (
        "AI_BEH_GoldAlloy_REL_Ratio_DOM_lin_640.npy"
    ),
    "AI_BEH_SilverCopper_REL_Ratio_DOM_lin_640.tif": (
        "AI_BEH_SilverCopper_REL_Ratio_DOM_lin_640.npy"
    ),
    "AI_BEH_ERT_Resistivity_Proxy_DOM_lin_640.tif": (
        "AI_BEH_ERT_Resistivity_Proxy_DOM_lin_640.npy"
    ),
}

AI_BEH_EXTENDED_REQUIRED_BANDS: tuple[str, ...] = ("B2", "B4", "B8", "B11", "B12")

AI_BEH_EXTENDED_DEFAULT_OUTPUT_DIR = "semantic_features/ai_beh_extended"
...
def compute_ai_beh_relation_features(
    ...
    *,
    ...
    arrays = _validated_band_arrays(
        bands,
        required_bands=AI_BEH_RELATION_REQUIRED_BANDS,
        family_label="AI_BEH relation",
    )
    return {
...
def compute_ai_beh_extended_features(
    bands: Mapping[str, object],
    *,
    denominator_epsilon: float = 1e-6,
) -> dict[str, np.ndarray]:
    """Compute the private AI_BEH extended feature family from 2D bands."""

    arrays = _validated_band_arrays(
        bands,
        required_bands=AI_BEH_EXTENDED_REQUIRED_BANDS,
        family_label="AI_BEH extended",
    )
    return {
        "AI_BEH_GoldAlloy_REL_Ratio_DOM_lin_640.tif": _safe_divide(
            arrays["B12"],
            arrays["B11"],
            denominator_epsilon=denominator_epsilon,
        ),
        "AI_BEH_SilverCopper_REL_Ratio_DOM_lin_640.tif": _safe_divide(
            arrays["B4"],
            arrays["B2"],
            denominator_epsilon=denominator_epsilon,
        ),
        "AI_BEH_ERT_Resistivity_Proxy_DOM_lin_640.tif": _safe_divide(
            arrays["B8"] + arrays["B4"],
            arrays["B11"] + 0.001,
            denominator_epsilon=denominator_epsilon,
        ),
...
def write_ai_beh_extended_feature_npy_outputs(
    run_dir: str | Path,
    bands: Mapping[str, object],
    *,
    reference_profile: Mapping[str, object] | None = None,
    output_relative_dir: str | Path = AI_BEH_EXTENDED_DEFAULT_OUTPUT_DIR,
    denominator_epsilon: float = 1e-6,
) -> dict[str, object]:
    """Write private local NPY extended features under ``run_dir`` only."""

    output_dir = resolve_run_output_path(run_dir, output_relative_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    features = compute_ai_beh_extended_features(
        bands,
        denominator_epsilon=denominator_epsilon,
    )

    output_paths: dict[str, str] = {}
    for tif_name in AI_BEH_EXTENDED_OUTPUT_NAMES:
        npy_name = AI_BEH_EXTENDED_NPY_OUTPUT_NAMES[tif_name]
        output_path = resolve_run_output_path(
            run_dir,
            Path(output_relative_dir) / npy_name,
        )
        np.save(output_path, features[tif_name])
        output_paths[tif_name] = str(output_path)

    return {
        "writer_family": "ai_beh_extended_semantic_features",
        "target_mode": "notebook_parity_private",
        "artifact_class": "LOCAL_SENSITIVE",
        ...
        "dtype_policy": "float32 with NaN for unsafe division denominators",
        "nodata_policy": (
            "NaN in NPY outputs; frozen references required before TIFF nodata lock"
        ),
    }


def _validated_band_arrays(
    bands: Mapping[str, object],
    *,
    required_bands: tuple[str, ...],
    family_label: str,
) -> dict[str, np.ndarray]:
    missing = [name for name in required_bands if name not in bands]
    if missing:
        raise ValueError(f"missing required bands: {', '.join(missing)}")

    arrays = {
        name: np.asarray(bands[name], dtype=np.float64) for name in required_bands
    }

    shapes = {array.shape for array in arrays.values()}
    if any(array.ndim != 2 for array in arrays.values()):
        raise ValueError(f"{family_label} feature inputs must be 2D raster arrays")
    if len(shapes) != 1:
        raise ValueError(f"{family_label} feature inputs must share the same 2D shape")
