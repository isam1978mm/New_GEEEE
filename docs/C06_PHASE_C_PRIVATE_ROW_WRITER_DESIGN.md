...
This document defines the design for a later C06 private hard-negative I1 row writer.
...
The future C06 row writer should create private hard-negative I1 rows outside Git, matching the sampling policy defined in Phase B.
...
```text
scripts/c06_write_private_i1_rows.py
```
...
```text
write private files under C:\Dev\New_GEE_PRIVATE\I1_C06
print aggregate JSON summary
```
...
```text
c06_private_row_writer_design_ready
```
...
```text
Create C06 private sample manifest generator or C06 private row writer script
```

Because C06 needs a private candidate source before rows can be written, the safest next implementation is a generator or writer that refuses `--write` until private C06 sample input exists.
