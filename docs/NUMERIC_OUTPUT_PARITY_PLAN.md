...
- DEM: 5 outputs pass
- DEM: `hillshade_0to1_640.tif` passes after a notebook-expression reference refresh
- SAR: 8 band outputs pass
- STACKS: `FINAL_TESLA_V7_2_HYPERCUBE.tif` passes
- STACKS: `FINAL_TESLA_V7_2_HYPERCUBE.npy` passes
- STACKS: `FINAL_TESLA_V7_2_HYPERCUBE_PATCHED_14B.tif` passes
- STACKS: `RADAR_STACK_HWC_640_app.npy` is an accepted classified strict `xfail` due inherited SAR dB residual
...
- frozen `RADAR_STACK_HWC` already equals `np.stack` of the frozen SAR band references exactly
- app `RADAR_STACK_HWC_640_app.npy` already equals `np.stack` of the app SAR band outputs exactly
- the remaining `RADAR_STACK_HWC` mismatch is inherited entirely from the app-vs-frozen SAR dB band residual, not from stack assembly
- no `RADAR_STACK` reference refresh is needed or approved
...
- no SAR math, SAR reference refresh, or tolerance change is approved for the remaining `RADAR_STACK_HWC` xfail
...
- upstream SAR dB residual for `RADAR_STACK_HWC_640_app.npy`

## Hillshade Reference Refresh Closeout

Current status:

- the old frozen hillshade reference was inconsistent with the notebook Cell 13 hillshade expression applied to frozen `DEM_640.tif`
- an approved one-file reference refresh regenerated `DEM_GEO8_TIFS/hillshade_0to1_640.tif` from the notebook expression only
- strict zero-tolerance DEM parity now passes for all six required DEM outputs
- no DEM math or tolerance change was needed
...
- `RADAR_STACK_HWC_640_app.npy`: accepted classified strict `xfail` due inherited SAR dB band residual
...
- DEM hillshade: reference refresh completed and parity passing
