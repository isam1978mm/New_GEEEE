...
Last update: 2026-07-03 - Strict B1 frozen-reference audit found zero notebook refs for #8/#9/#15/#17/#29; no Full parity promotions.

## Plan B scope: not-done-now items

| # | Not-done-now notebook item | Is it done now? | Can we make it in app? | Plan B note |
|---:|---|---|---:|---|
| 8 | Nano / treasure / geophysics stacks | Partial / no strict notebook ref found | Yes | App stack/order proof is clean. Strict frozen-reference audit found no notebook reference for NANO_GEOPHYSICS_STACK_640.npy or TREASURE_GEOPHYSICS_STACK_640.npy, so Full notebook numeric parity remains blocked. |
| 9 | More feature stacks / rename layers | Partial / no strict notebook ref found | Yes | App stack/order proof is clean. Strict frozen-reference audit found no notebook references for the six #9 stack outputs, so Full notebook numeric parity remains blocked. |
| 15 | Bonus / simulator features | Partial / no strict notebook ref found | Yes | App stack/order proof is clean. Strict frozen-reference audit found no notebook references for AUX_BONUS_FEATURES_STACK_640.npy or SIM_GEOPHYSICAL_STACK_640.npy, so Full notebook numeric parity remains blocked. |
| 17 | Extra S2 era pulls / masks | Partial / no strict notebook ref found | Yes | App stack/order proof is clean for AIX_2022_2026_CLOUDLT3_EXTRA_TENSORS_STACK_640.npy. Strict frozen-reference audit found no notebook reference, so Full notebook numeric parity remains blocked. |
...
| 29 | AI tensor builder for YOLO/CNN/Swin/SegFormer | Partial / no strict notebook ref found | Yes | App-output proof is clean for AI_TENSORS_STAGE4 outputs. Strict frozen-reference audit found no notebook references for the full tensor, YOLO, CNN, Swin/SegFormer, PCA, negative mask, or YOLO visual outputs, so Full notebook numeric parity remains blocked. No model training/inference/weights/dependency changes. |
...
| 38 | Live geemap overlays | App-native replacement / output proof clean | Replace | App-native replacement implemented for canonical cell 243 as full_job/focus/APP_NATIVE_LIVE_OVERLAY_MANIFEST_V7_2.json plus operator-only coordinate-free preview family. B1 inspection confirmed the manifest exists, source_cell is cell_243, and focused tests passed. No geemap port, public tiles, public coordinates, public route outputs, HTTP download exposure, or frontend public exposure are approved. Full live notebook parity remains blocked unless a later explicit operator/private overlay approval allows original live-map behavior under gates. |
