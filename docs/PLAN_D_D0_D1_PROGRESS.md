...
```text
D0 remaining:
  - hypercube invalid values do not become finite signal
  - object extraction invalid-only candidate guard test
  - focus stats nodata sentinel test
  - SAR angle nodata validity test

D1 remaining:
  - patch hypercube.py so invalid source pixels are not normalized as 0.0
  - make hypercube valid_mask policy explicit in metadata/QA
...
The hypercube bug is still the largest remaining D1 blocker. The PCA/classifier patches reduce downstream damage from valid_mask-aware runs, but they do not fully fix the root hypercube invalid-pixel normalization issue yet.
