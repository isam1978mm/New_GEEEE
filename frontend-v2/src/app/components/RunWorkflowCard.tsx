...
export function RunWorkflowCard({ onQueueRun, onPreviewRoi, onPlanEarthEngine, isQueueing = false, feedback = null, externalTilesEnabled = false, tileUrlTemplate = "" }: RunWorkflowCardProps) {
  ...
  const [mapCenter, setMapCenter] = useState<MapPoint>({ lat: 0, lon: 0 });
  ...
  const grid = useMemo(
    () => externalTilesEnabled && templateOk ? buildTiles(tileUrlTemplate, mapCenter.lat, mapCenter.lon, mapZoom) : null,
    [externalTilesEnabled, templateOk, tileUrlTemplate, mapCenter.lat, mapCenter.lon, mapZoom],
  );
  ...
  function chooseTarget(point: MapPoint) {
  ...
  function handleMapClick(event: MouseEvent<HTMLButtonElement>) {
    ...
    const displayTileX = rect.width / GRID_SIZE;
    const displayTileY = rect.height / GRID_SIZE;
    const dx = event.clientX - rect.left - rect.width / 2;
    const dy = event.clientY - rect.top - rect.height / 2;
    chooseTarget(pixelToLatLon(grid.centerPixel.x + dx * (TILE_SIZE / displayTileX), grid.centerPixel.y + dy * (TILE_SIZE / displayTileY), grid.zoom));
  }

  function panMap(dxTiles: number, dyTiles: number) {
    const centerPixel = latLonToPixel(mapCenter.lat, mapCenter.lon, mapZoom);
    setMapCenter(pixelToLatLon(centerPixel.x + dxTiles * TILE_SIZE, centerPixel.y + dyTiles * TILE_SIZE, mapZoom));
  }

  function zoomMap(delta: number) {
    setMapZoom((current) => Math.max(2, Math.min(18, current + delta)));
  }
  ...
  function handleReset() {
    setLatitude(""); setLongitude(""); setRunName(""); setBufferKm("2.0"); setResolution("640"); setShowAdvanced(false); setMapCenter({ lat: 0, lon: 0 }); setMapZoom(DEFAULT_ZOOM); clearComputed();
  }
  ...
  return (
    <div className="rounded-lg bg-card flex flex-col" style={{ border: "1px solid var(--border)", boxShadow: "0 1px 3px rgba(28,43,94,0.06)" }}>
      ...
      <div className="px-4 py-3 flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1"><div className="flex items-center justify-between"><span style={{ fontSize: "11px", fontWeight: 600, color: "var(--gs-navy)" }}>Large map target picker</span><span style={{ fontSize: "10px", color: "var(--gs-slate)" }}>{externalTilesEnabled ? `zoom ${mapZoom}` : "tiles disabled"}</span></div><BigMapPicker grid={grid} target={targetOk ? { lat, lon } : null} centerLoaded={centerLoaded} centerFailed={centerFailed} tileStates={tileStates} externalTilesEnabled={externalTilesEnabled} templateOk={templateOk} onClick={handleMapClick} onTileLoad={(key) => setTileStates((current) => ({ ...current, [key]: "success" }))} onTileError={(key) => setTileStates((current) => ({ ...current, [key]: "error" }))} onPan={panMap} onZoom={zoomMap} onRecenter={() => targetOk && setMapCenter({ lat, lon })} /></div>
...
function BigMapPicker({ grid, target, centerLoaded, centerFailed, tileStates, externalTilesEnabled, templateOk, onClick, onTileLoad, onTileError, onPan, onZoom, onRecenter }: { grid: ReturnType<typeof buildTiles> | null; target: MapPoint | null; centerLoaded: boolean; centerFailed: boolean; tileStates: Record<string, TileLoadState>; externalTilesEnabled: boolean; templateOk: boolean; onClick: (event: MouseEvent<HTMLButtonElement>) => void; onTileLoad: (key: string) => void; onTileError: (key: string) => void; onPan: (dxTiles: number, dyTiles: number) => void; onZoom: (delta: number) => void; onRecenter: () => void }) {
  if (!externalTilesEnabled) return <MapNotice title="Map disabled" detail="Enable External map tiles in Settings to open the large navigable map picker." />;
  if (!templateOk) return <MapNotice title="Map unavailable" detail="Tile URL template must include {z}, {x}, and {y}." />;
  if (!grid) return <MapNotice title="Map loading" detail="Preparing map tiles." />;
  const pin = target ? targetPosition(grid, target) : null;
  const pinVisible = pin && pin.left >= 0 && pin.left <= 100 && pin.top >= 0 && pin.top <= 100;
  return <div className="rounded overflow-hidden" style={{ border: "1px solid rgba(28,43,94,0.14)", backgroundColor: "rgba(248,247,242,0.95)" }}><div className="flex items-center justify-between gap-2 px-2 py-1.5" style={{ borderBottom: "1px solid rgba(28,43,94,0.12)", backgroundColor: "rgba(255,255,255,0.78)" }}><span style={{ fontSize: 11, color: "var(--gs-slate)" }}>Pan and zoom, then click the map to place the red target pin.</span><div className="flex items-center gap-1"><MapButton label="−" onClick={() => onZoom(-1)} /><MapButton label="+" onClick={() => onZoom(1)} /><MapButton label="↑" onClick={() => onPan(0, -PAN_FRACTION)} /><MapButton label="↓" onClick={() => onPan(0, PAN_FRACTION)} /><MapButton label="←" onClick={() => onPan(-PAN_FRACTION, 0)} /><MapButton label="→" onClick={() => onPan(PAN_FRACTION, 0)} /><MapButton label="Target" onClick={onRecenter} /></div></div><button type="button" onClick={onClick} className="block" style={{ position: "relative", width: "100%", height: 560, overflow: "hidden", border: "none", padding: 0, cursor: "crosshair", backgroundColor: "rgba(248,247,242,0.95)" }}>{!centerFailed && <div style={{ position: "absolute", inset: 0, display: "grid", gridTemplateColumns: `repeat(${GRID_SIZE}, 1fr)`, gridTemplateRows: `repeat(${GRID_SIZE}, 1fr)` }}>{grid.tiles.map((tile) => <div key={tile.key} style={{ position: "relative", overflow: "hidden" }}><img src={tile.url} alt={tile.isCenter ? "Target map center tile" : "Target map surrounding tile"} onLoad={() => onTileLoad(tile.key)} onError={() => onTileError(tile.key)} style={{ width: "100%", height: "100%", objectFit: "cover", display: tileStates[tile.key] === "error" ? "none" : "block", opacity: tileStates[tile.key] === "loading" ? 0.45 : 1 }} />{tileStates[tile.key] === "error" && <span style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, color: "var(--gs-slate)" }}>tile unavailable</span>}</div>)}</div>}{centerFailed && <MapNotice title="Tile preview failed" detail="The configured tile URL did not return the center map image." absolute />}{tileStates[grid.center.key] === "loading" && !centerLoaded && !centerFailed && <MapNotice title="Loading map..." detail="Waiting for the center tile image." absolute />}{centerLoaded && !target && <span style={{ position: "absolute", left: "50%", top: "50%", transform: "translate(-50%, -50%)", fontSize: 12, color: "var(--gs-navy)", backgroundColor: "rgba(255,255,255,0.9)", padding: "5px 8px", borderRadius: 4 }}>Click map to place target pin</span>}{pinVisible && <div style={{ position: "absolute", left: `${pin.left}%`, top: `${pin.top}%`, transform: "translate(-50%, -100%)", pointerEvents: "none", textAlign: "center" }}><MapPin size={34} style={{ color: "var(--gs-red)", filter: "drop-shadow(0 2px 3px rgba(28,43,94,0.35))" }} /><div className="font-mono" style={{ fontSize: 9, fontWeight: 700, color: "var(--gs-navy)", backgroundColor: "rgba(255,255,255,0.92)", borderRadius: 3, padding: "1px 5px" }}>Target</div></div>}</button><div className="px-2.5 py-1.5" style={{ fontSize: 10, color: "var(--gs-slate)", borderTop: "1px solid rgba(28,43,94,0.12)", display: "flex", justifyContent: "space-between" }}><span>{grid.providerLabel}</span><span className="font-mono">z{grid.zoom} center x{grid.center.x} y{grid.center.y}</span></div></div>;
}

function MapButton({ label, onClick }: { label: string; onClick: () => void }) { return <button type="button" onClick={onClick} style={{ fontSize: 11, color: "var(--gs-navy)", backgroundColor: "white", border: "1px solid rgba(28,43,94,0.16)", borderRadius: 4, padding: "3px 7px", cursor: "pointer" }}>{label}</button>; }
function MapNotice({ title, detail, absolute = false }: { title: string; detail: string; absolute?: boolean }) { return <div className="flex flex-col items-center justify-center gap-1 px-4 py-6" style={{ minHeight: absolute ? undefined : 360, position: absolute ? "absolute" : "relative", inset: absolute ? 0 : undefined, textAlign: "center" }}><div style={{ fontSize: 13, fontWeight: 700, color: "var(--gs-navy)" }}>{title}</div><div style={{ fontSize: "11px", color: "var(--gs-slate)", maxWidth: 360 }}>{detail}</div></div>; }
...
function Panel({ title, subtitle, icon, label, disabled, onClick, children }: { title: string; subtitle: string; icon: ReactNode; label: string; disabled: boolean; onClick: () => void; children: ReactNode }) { return <div className="rounded px-3 py-2 flex flex-col gap-2" style={{ backgroundColor: "rgba(255,255,255,0.72)", border: "1px solid rgba(28,43,94,0.12)" }}><div className="flex items-center justify-between gap-2"><div><div className="font-mono" style={{ fontSize: "9.5px", fontWeight: 700, color: "var(--gs-navy)", textTransform: "uppercase" }}>{title}</div><div style={{ fontSize: "10.5px", color: "var(--gs-slate)" }}>{subtitle}</div></div><button type="button" disabled={disabled} onClick={onClick} className="flex items-center gap-1.5 rounded px-2.5 py-1.5" style={{ backgroundColor: disabled ? "var(--muted)" : "var(--gs-navy)", color: disabled ? "var(--gs-slate)" : "white", border: "none", cursor: disabled ? "not-allowed" : "pointer", fontSize: "11px", fontWeight: 600 }}>{icon}{label}</button></div>{children}</div>; }
...
function buildTiles(template: string, latitude: number, longitude: number, zoom: number) {
  ...
  const tiles = Array.from({ length: GRID_SIZE }, (_, yIndex) => yIndex - half).flatMap((yo) => Array.from({ length: GRID_SIZE }, (_, xIndex) => xIndex - half).map((xo) => { const x = clampTile(center.x + xo, zoom); const y = clampTile(center.y + yo, zoom); return { key: `${zoom}-${x}-${y}`, x, y, isCenter: xo === 0 && yo === 0, url: template.replaceAll("{z}", String(zoom)).replaceAll("{x}", String(x)).replaceAll("{y}", String(y)) }; }));
  return { providerLabel: tileLabel(template), zoom, center, centerPixel, tiles };
}
function targetPosition(grid: ReturnType<typeof buildTiles>, target: MapPoint) { const pixel = latLonToPixel(target.lat, target.lon, grid.zoom); return { left: 50 + ((pixel.x - grid.centerPixel.x) / TILE_SIZE) * (100 / GRID_SIZE), top: 50 + ((pixel.y - grid.centerPixel.y) / TILE_SIZE) * (100 / GRID_SIZE) }; }
