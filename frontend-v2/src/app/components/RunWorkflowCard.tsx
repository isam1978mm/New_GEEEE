...
export function RunWorkflowCard({ onQueueRun, onPreviewRoi, onPlanEarthEngine, isQueueing = false, feedback = null, externalTilesEnabled = false, tileUrlTemplate = "" }: RunWorkflowCardProps) {
  ...
  function updateLatitude(value: string) { setLatitude(value); clearComputed(); }
  function updateLongitude(value: string) { setLongitude(value); clearComputed(); }
  function handleTargetChange(target: { lat: number; lon: number }) { setLatitude(target.lat.toFixed(8)); setLongitude(target.lon.toFixed(8)); clearComputed(); }

  async function handlePreviewRoi() {
    if (!canPreviewRoi) return;
    setIsPreviewingRoi(true);
    setRoiPreviewError(null);
    try { setRoiPreview((await onPreviewRoi?.({ lat, lon })) ?? null); }
    catch (error) { setRoiPreview(null); setRoiPreviewError(error instanceof Error ? error.message : "Preview request failed."); }
    finally { setIsPreviewingRoi(false); }
  }
  ...
  async function handlePlanEarthEngine() {
    ...
    } catch (error) { setEePlan(null); setEePlanError(error instanceof Error ? error.message : "Planning request failed."); }
    finally { setIsPlanningEe(false); }
  }
  ...
  return (
    <div className="rounded-lg bg-card flex flex-col" style={{ border: "1px solid var(--border)", boxShadow: "0 1px 3px rgba(28,43,94,0.06)" }}>
      ...
      <div className="px-4 py-3 flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-2.5"><Coord label="Latitude" value={latitude} min={-90} max={90} placeholder="e.g. 43.6532" invalid={hasLat && !latOk} onChange={updateLatitude} /><Coord label="Longitude" value={longitude} min={-180} max={180} placeholder="e.g. -79.3832" invalid={hasLon && !lonOk} onChange={updateLongitude} /></div>
        {((hasLat && !latOk) || (hasLon && !lonOk)) && <Message text="Latitude must be -90 to 90 and longitude must be -180 to 180." />}
        <div className="flex flex-col gap-1"><div className="flex items-center justify-between"><span style={{ fontSize: "11px", fontWeight: 600, color: "var(--gs-navy)" }}>Large map target picker</span><span style={{ fontSize: "10px", color: "var(--gs-slate)" }}>{externalTilesEnabled ? "mouse drag + scroll zoom" : "tiles disabled"}</span></div><TargetLeafletMap externalTilesEnabled={externalTilesEnabled} tileUrlTemplate={tileUrlTemplate} target={targetOk ? { lat, lon } : null} onTargetChange={handleTargetChange} /></div>
