...
# Goal F16 - Reconcile SAR processing math after F15 diagnosis

Reason:

F15 ran successfully after F14 source-selection parity was fixed.

F15 proved:

- notebook and app now use the same SAR source image pairs
- mask_overlap_percent is 100% for VV/VH/logRatio/incidence
- notebook logRatio equals VV_dB - VH_dB
- app logRatio also equals VV_dB - VH_dB
- logRatio mismatch is downstream from VV/VH mismatch
- radar_linear_support_stack mismatch is downstream from SAR band mismatch
- the remaining root mismatch is SAR VV/VH/angle-incidence processing

F15 reported:

- VV_dB_raster: PROCESSING_ALGORITHM_MISMATCH
- VH_dB_raster: PROCESSING_ALGORITHM_MISMATCH
- logRatio_dB: UPSTREAM_VV_VH_PROCESSING_MISMATCH
- incidence/angle: PROCESSING_ALGORITHM_MISMATCH
- radar stack: DOWNSTREAM_FROM_SAR_BANDS

F16 must reconcile the SAR processing math/path after source-selection identity has already been proven.

Do not change SAR pair-selection unless a test proves F14 regressed.
Do not change notebook code.
Do not weaken tolerances.
Do not mark mismatched SAR outputs as PASS.
Do not work on downstream stacks until VV/VH/angle processing is resolved.

Scope:

- SAR processing math/path only.
- Focus on VV_dB, VH_dB, angle/incidence, and logRatio construction.
- Use F15 report as the starting evidence.
- Keep radar_linear_support_stack as downstream diagnostic only.
- Do not work on DEM derivatives, hypercube, object extraction, field ops, GPS, or classifier logic.

Required investigation:

- Inspect notebook SAR processing code or exported metadata to determine exact processing path:
  - DBONLY_LOCALDEM meaning
  - local DEM RTC correction formula
  - whether VV/VH are processed in dB or linear units
  - whether aggregation is mean/median/mosaic/qualityMosaic
  - whether aggregation occurs before or after dB conversion
  - whether angle is taken from master image, averaged image, first pair, or final composite
  - whether masking is applied before/after aggregation
  - whether clip/sample order differs
- Compare app implementation in app/pipeline/stages/sar_rtc.py against notebook behavior.
- Add narrow diagnostics if current metadata is insufficient.

Create/update:

- app/pipeline/stages/sar_rtc.py only for proven SAR processing mismatch fixes
- app/services/sar_processing_parity.py if more evidence fields are needed
...
Required F16 diagnosis outputs:

- Document which processing difference is proven:
  - aggregation order
  - dB/linear conversion order
  - local DEM RTC correction path
  - angle source
  - mask/nodata order
  - raster sampling/order
- Add tests showing the proven mismatch and the corrected behavior.
- Preserve F13 pair-selection MATCH behavior.

Allowed reconciliation:

- Add a SAR processing profile such as notebook_parity if needed.
- Change VV/VH processing order only if notebook evidence proves it.
- Change logRatio construction only if notebook evidence proves it, but F15 currently shows both sides use VV - VH correctly.
- Change angle/incidence source or aggregation only if notebook evidence proves it.
- Change nodata/masking order only if F15/F16 evidence proves it.
- Add local-only diagnostics explaining any remaining mismatch.

Not allowed in F16:

- Do not change SAR pair-selection rules except for proven regression.
- Do not change DEM derivative, hypercube, tensor, object extraction, field ops, GPS, or classifier logic.
- Do not weaken tolerances.
- Do not change notebook code.
- Do not expose coordinates, geometry, local paths, CRS transforms, hashes, or exact ROI context through public API responses.
- Do not serve F11/F13/F15/F16 reports over HTTP.
...
- Run F13 source-selection report and confirm:
  - image_identity MATCH
  - orbit_pairing MATCH
  - vv_vh_pair_count MATCH
  - source_parameters MATCH
...
- If numeric parity still fails, report exact remaining cause and next action.
...
# Goal F23 - Diagnose true SAR processing delta after source-ID match

Reason:

F22 added true Cell 25 SAR pair provenance support.

After running the F22 helper on the latest validation run, F13 with the Cell 25 sidecar proved:
...
Therefore the remaining VV/VH numeric delta is no longer attributable to missing or mismatched Sentinel-1 source identity.

F20 ruled out nodata, edge-only behavior, and angle-driven RTC as the main cause.
F21 showed the residual is balanced, nonlinear, partly outlier-driven, and VV/VH asymmetric.

F23 must diagnose the true SAR processing delta after source-ID match.

Scope:

- SAR processing diagnostics only.
- Source identity is considered proven when a Cell 25 sidecar is supplied and F13 reports SOURCE_ID_MATCH_PROCESSING_DELTA_REMAINS.
- Do not change pair-selection constants.
- Do not change selected image IDs.
- Do not change notebook code or notebook output arrays.
- Do not weaken tolerances.
- Do not mark mismatched outputs as PASS.
- Do not change SAR math unless an exact notebook-code-backed processing mismatch is proven.

Required setup:

- Use latest validation run:
  - run id 68ec6b07-1842-4957-88cc-1b259b09dfdb
- Use Cell 25 pair sidecar generated by F22:
  - QA_RADAR_CELL25_PAIR_IDS_68ec6b07-1842-4957-88cc-1b259b09dfdb_pairs4_pairdt36h_orbitpm9d.json
- Require F13 source_identity_classification to be SOURCE_ID_MATCH_PROCESSING_DELTA_REMAINS before interpreting SAR processing residuals as true processing deltas.

Required investigation:

Focus on the remaining true processing delta:

- Earth Engine median/reproject/sampleRectangle semantics
- sigma Lee implementation details
- Lee fallback/noise variance behavior
- dB-to-linear-to-dB precision and float casting
- per-image versus per-pair reproject timing
- pair median versus final median evaluation order
- tile/sampleRectangle assembly and dtype conversion
- sparse outlier group above 0.1 dB
- VV/VH asymmetric residual behavior

Required diagnostics:

Add local-only diagnostics that can answer group above 0.1 dB
- VV/VH asymmetric residual behavior

Required diagnostics:

Add local-only diagnostics that can answer:

- Are large residual pixels spatially clustered or distributed?
  - use row/col bin counts only, no coordinates
- Are large residual pixels related to high slope, high incidence, low backscatter, or filter threshold regions?
  - count-only bins only
- Do residuals change if comparing only low-slope or mid-incidence pixels?
- Are residuals concentrated near the sigma Lee threshold boundary?
- Are residuals concentrated in one band or different between VV/VH?
- Does casting to float32 before or after median/RTC explain any residual?
- Does linear-domain median versus dB-domain median explain any residual?
  - diagnostic-only unless notebook code proves the order

Allowed diagnostic-only profiles:

These must not affect normal app outputs unless a proven fix is later approved.

- dtype profile:
  - float64_current
  - float32_after_sample
  - float32_before_rtc
- median domain profile:
  - current notebook-backed path
  - linear_domain_median diagnostic only if notebook ambiguity remains
  - db_domain_median diagnostic only if notebook ambiguity remains
- sample/finalize profile:
  - current sampleRectangle assembly
  - tile-boundary diagnostic metrics
- filter threshold profile:
  - count pixels near sigma Lee low/high thresholds if intermediate values are available or can be locally reproduced

Potential narrow fix:

Only if F23 proves an exact notebook-code-backed mismatch, make one narrow change in app/pipeline/stages/sar_rtc.py.

Examples of acceptable fixes if proven:

- wrong float casting point
- wrong median domain/order
- wrong reproject timing
- wrong sample/finalize order
- wrong Lee/sigma Lee threshold handling

If no exact bug is proven, F23 must remain diagnostic-only.
...
- Do not expose coordinates, geometry, bounds, CRS transforms, hashes, local paths, or exact ROI context through public API responses.
- Do not serve parity reports over HTTP.
- Do not claim parity unless F11/F15 proves it.
...
- Rerun F13 with the F22 Cell 25 sidecar and confirm:
  - source_identity_classification SOURCE_ID_MATCH_PROCESSING_DELTA_REMAINS
- Rerun SAR processing parity report on latest validation run:
  - run id 68ec6b07-1842-4957-88cc-1b259b09dfdb
- Rerun F11 numeric parity only if SAR logic changes.
- Report:
  - whether F23 is diagnostic-only or includes a proven narrow fix
  - whether residual appears dtype/casting-driven, median-domain/order-driven, filter-threshold-driven, tile/sample-driven, or unresolved
  - exact next action
