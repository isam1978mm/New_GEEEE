"""Plan, dry-run, or write private C05 negative/background I1 rows.
...
This script does not download C05 data, assemble I2, run the readiness validator,
...
DEFAULT_SAMPLE_MANIFEST = Path(r"C:\Dev\New_GEE_PRIVATE\C05_RAW\c05_sample_manifest.private.jsonl")
DEFAULT_OUTPUT_DIR = Path(r"C:\Dev\New_GEE_PRIVATE\I1_C05")
DEFAULT_DATASET_ID = "c05_i1_private_v1"
DEFAULT_SCHEMA_VERSION = "training_example_v1"
DEFAULT_SOURCE_VERSION = "esa_worldcover_10m_operator_selected_local_version"
...
class C05InputError(ValueError):
    """Raised when the private C05 sample input is missing or invalid."""
...
def main() -> int:
    ...
    summary = {
        ...
        "planned_label_counts": {"Class_Background": len(selected_rows)},
        "planned_label_quality_counts": {"reviewed_independent": len(selected_rows)},
        "planned_evidence_type_counts": {"authoritative_external_dataset": len(selected_rows)},
        ...
        "notes": (
            "C05 is negative/background only. This writer requires a private sample "
    ...
    if args.write:
        if not ready_to_write:
            raise SystemExit(
                "C05 write refused: provide a valid private sample manifest with at least "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dry-run or write private C05 negative/background I1 rows outside Git."
    )
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise C05InputError(f"invalid JSON on line {line_number}") from exc
            if not isinstance(payload, dict):
                raise C05InputError(f"line {line_number} is not a JSON object")
            rows.append(payload)
    if not rows:
        raise C05InputError("sample manifest is empty")
    return rows
...
def _eligible_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not _has_text(row.get("source_record_ref")):
            continue
        if not _has_text(row.get("background_class")):
            continue
        eligible.append(row)
    return eligible
...
def _sample_sort_key(row: dict[str, Any], seed: int) -> str:
    payload = {
        ...
        "background_class": str(row.get("background_class", "")),
    }
    ...
    fingerprint = _stable_hash(
        {
            ...
            "background_class": sample.get("background_class"),
        }
    )
    short_id = fingerprint[:16]
    private_ref = f"c05_private_ref_{short_id}"
    row = {
        "schema_version": schema_version,
        "sample_id": f"c05_sample_{short_id}",
        "dataset_id": dataset_id,
        "area_id": f"c05_area_{short_id}",
        "group_id": f"c05_group_{short_id}",
        ...
        "evidence_review_method": "c05_private_background_sampling_policy_seeded",
        ...
        "notes": "C05 private background row; features, split policy, and I2 readiness remain pending.",
    }
...
def _write_private_outputs(
    *,
    ...
    _write_jsonl(output_dir / "training_examples.c05.private.jsonl", rows)
    _write_json(output_dir / "training_examples.c05.private.summary.json", summary)
    _write_json(
        output_dir / "source_lineage.c05.private.json",
        {
    ...
    _write_json(
        output_dir / "exclusions.c05.private.summary.json",
        {
            "dataset_id": summary["dataset_id"],
