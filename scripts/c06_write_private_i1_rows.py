"""Plan, dry-run, or write private C06 hard-negative I1 rows.
...
This script does not download C06 data, assemble I2, run the readiness validator,
...
DEFAULT_SAMPLE_MANIFEST = Path(r"C:\Dev\New_GEE_PRIVATE\C06_RAW\c06_sample_manifest.private.jsonl")
DEFAULT_OUTPUT_DIR = Path(r"C:\Dev\New_GEE_PRIVATE\I1_C06")
DEFAULT_DATASET_ID = "c06_i1_private_v1"
DEFAULT_SCHEMA_VERSION = "training_example_v1"
DEFAULT_SOURCE_VERSION = "dynamic_world_operator_selected_local_version"
...
class C06InputError(ValueError):
    """Raised when the private C06 sample input is missing or invalid."""
...
def main() -> int:
    ...
    summary = {
        ...
        "notes": (
            "C06 is hard-negative only. This writer requires a private sample "
    ...
    if args.write:
        if not ready_to_write:
            raise SystemExit(
                "C06 write refused: provide a valid private sample manifest with at least "
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dry-run or write private C06 hard-negative I1 rows outside Git."
    )
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise C06InputError(f"invalid JSON on line {line_number}") from exc
            if not isinstance(payload, dict):
                raise C06InputError(f"line {line_number} is not a JSON object")
            rows.append(payload)
    if not rows:
        raise C06InputError("sample manifest is empty")
    return rows
...
def _make_i1_row(
    *,
    ...
    private_ref = f"c06_private_ref_{short_id}"
    row = {
        "schema_version": schema_version,
        "sample_id": f"c06_sample_{short_id}",
        "dataset_id": dataset_id,
        "area_id": f"c06_area_{short_id}",
        "group_id": f"c06_group_{short_id}",
        ...
        "evidence_review_method": "c06_private_hard_negative_sampling_policy_seeded",
        ...
        "notes": "C06 private hard-negative row; features, split policy, and I2 readiness remain pending.",
    }
...
def _write_private_outputs(
    *,
    ...
    _write_jsonl(output_dir / "training_examples.c06.private.jsonl", rows)
    _write_json(output_dir / "training_examples.c06.private.summary.json", summary)
    _write_json(
        output_dir / "source_lineage.c06.private.json",
        {
    ...
    _write_json(
        output_dir / "exclusions.c06.private.summary.json",
        {
            "dataset_id": summary["dataset_id"],
