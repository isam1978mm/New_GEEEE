"""Run Campaign 012 with a hard watchdog around each live ATL08 tile query.

Campaign 011 demonstrated that the underlying SlideRule ATL08 request can stall
without a wall-clock timeout. This Campaign-012-specific launcher installs the
...
class Campaign012TileTimeoutError(RuntimeError):
    """Raised when one Campaign 012 ATL08 tile exceeds its wall-clock limit."""
...
def _query_atl08_with_timeout(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="campaign012_atl08_") as temp_name:
        ...
        try:
            ...
            raise Campaign012TileTimeoutError(
                "Campaign 012 ATL08 tile query exceeded "
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                "Campaign 012 ATL08 worker exited without a result "
                f"(exit code {completed.returncode})"
            )
        if not result_path.is_file():
            raise RuntimeError("Campaign 012 ATL08 worker produced no result file")
...
def main() -> int:
    ...
    return campaign012.main()


if __name__ == "__main__":
    if len(sys.argv) >= 2 and sys.argv[1] == WORKER_FLAG:
        if len(sys.argv) != 5:
            print("Campaign 012 ATL08 worker received invalid arguments", file=sys.stderr)
            raise SystemExit(2)
        raise SystemExit(
            _worker_main(
                Path(sys.argv[2]),
