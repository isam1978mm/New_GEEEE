...
def _load_candidate_payload(path: Path) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != OPERATOR_CANDIDATES_SCHEMA:
        raise ValueError("unsupported operator candidate input schema")
    ...
    for raw_candidate in raw_candidates:
        ...
        canonical.append(candidate.as_mapping())
...
def run_operator_depth_for_existing_run(
    *,
    run_dir: Path,
    package_dir: Path,
    ...
    """Write private operator-calibrated depth outputs for a completed run."""
    ...
    if not isinstance(package, OperatorInterpolationPackage):
        raise ValueError("package is not an operator interpolation package")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run private operator-calibrated depth on an existing run.",
    )
