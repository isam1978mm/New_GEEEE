...
from app.pipeline.depth.recorded import RecordedDepthPackage
...
def _load_candidate_payload(path: Path) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != DEPTH_CANDIDATES_SCHEMA:
        raise ValueError("unsupported recorded-depth candidate input schema")
...
def run_recorded_depth_lookup_for_existing_run(
    *,
    run_dir: Path,
    package_dir: Path,
    ...
    """Report reviewed recorded depths for exact zone IDs on an existing run."""
    ...
    if not isinstance(package, RecordedDepthPackage):
        raise ValueError("package is not a recorded depth lookup package")
    candidate_payload = _load_candidate_payload(candidate_input)

    destination = run_dir / DEPTH_INPUT_RELATIVE_PATH
    if destination.exists() and not force:
        raise FileExistsError("run already has depth candidate input; use --force to replace it")
    ...
    return {
        ...
        "recorded_measurement_count": summary["recorded_measurement_count"],
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run reviewed recorded-depth lookup on an existing run.",
    )
