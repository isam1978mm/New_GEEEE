"""Run Campaign 008 inside recently released FDEP reclamation units.

Campaign 007 proved that an official mine-boundary constraint prevents broad
search candidates from landing on unrelated land, but its one finalized
candidate was inside a future-work unit with no documented event-window
activity. Campaign 008 tightens the search to official FDEP mandatory phosphate
reclamation units released during the ICESat-2 era (2019 through 2024).

The existing Campaign 007 temporal, stability, neighbour, cluster, and context
thresholds remain unchanged. This compatibility entry point changes only the
official polygon source and adds a unit-integrity gate: every supporting ATL08
segment in a surviving cluster must share exactly one named released unit.

A surviving cluster is still only a persistent terrain-step candidate. It does
not prove engineered fill, placed-material thickness, buried-object depth, or
...
CAMPAIGN_DESCRIPTION = (
    "Eighth independent ATL08 terrain-step campaign constrained to official "
    "FDEP mandatory phosphate reclamation units released from 2019 through "
    "2024, with all prior temporal, stability, context, and evidence gates "
    "preserved."
)
REGION_ID = "fdep_recent_released_phosphate_units"
REGION_DESCRIPTION = (
    "Central Florida mandatory phosphate reclamation units with official "
    "release years from 2019 through 2024; tiles, ATL08 segments, and final "
    "clusters are constrained to named released-unit geometry."
)
FDEP_RELEASED_UNITS_LAYER_URL = (
    "https://ca.dep.state.fl.us/arcgis/rest/services/OpenData/"
    "MMP_MANPHO/MapServer/14/query"
)
RELEASE_YEAR_MIN = 2019
RELEASE_YEAR_MAX = 2024
DEFAULT_BOUNDS = (-82.70, 27.10, -81.45, 28.65)
POLYGON_CACHE_SCHEMA = "icesat2_fdep_recent_released_unit_tile_cache_v1"
UNIT_GEOJSON_FILENAME = "fdep_recent_released_units.geojson"

FetchJson = Callable[[str, dict[str, str], float], dict[str, Any]]

_ORIGINAL_REGION_RESULT = campaign._region_result
_ORIGINAL_SUMMARY = campaign._summary


def _release_year(feature: dict[str, Any]) -> int | None:
    properties = feature.get("properties")
    if not isinstance(properties, dict):
        return None
    value = properties.get("RELEASE_YEAR")
...
def fetch_recent_released_units(
    *,
    west: float,
    south: float,
    east: float,
    ...
    """Fetch official released units in the bounds and release-year window."""
    ...
    payload = fetch_json(
        ...
        {
            "where": (
                f"RELEASE_YEAR >= {RELEASE_YEAR_MIN} AND "
                f"RELEASE_YEAR <= {RELEASE_YEAR_MAX}"
            ),
            "geometry": envelope,
    ...
    if payload.get("exceededTransferLimit") is True:
        raise ValueError("FDEP released-unit query exceeded the transfer limit")
    ...
    for feature in raw_features:
        ...
        if not isinstance(geometry, dict) or geometry.get("type") not in {
            ...
            continue
        year = _release_year(feature)
        if year is None or not RELEASE_YEAR_MIN <= year <= RELEASE_YEAR_MAX:
            continue
        features.append(feature)
    if not features:
        raise ValueError(
            "no official recently released reclamation units intersect the bounds"
        )
...
def _unit_identity(feature: dict[str, Any]) -> str:
    ...
    return " | ".join(
        str(values.get(key, ""))
        for key in ("MINE_NAME", "SITE_ID", "REC_UNITS", "RELEASE_YEAR", "OBJECTID")
    )
...
def _unit_metadata(feature: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "reclamation_status": values.get("REC_STATUS"),
        "release_status": values.get("RELEASESTATUS"),
        "release_year": _release_year(feature),
...
def shared_released_unit(
    cluster: dict[str, Any],
    feature_collection: dict[str, Any],
) -> dict[str, Any] | None:
    """Return one common released unit for every cluster segment, if exact."""
...
def filter_clusters_to_single_unit(
    ...
    for cluster in clusters:
        metadata = shared_released_unit(cluster, feature_collection)
        if metadata is None:
            rejections.append(
                {
                    "reason": "supporting_segments_do_not_share_one_released_unit",
            ...
            continue
        item = dict(cluster)
        item["official_released_unit"] = metadata
        survivors.append(item)
    return survivors, rejections


def _recent_unit_labels(feature_collection: dict[str, Any]) -> list[str]:
    raw_features = feature_collection.get("features")
    if not isinstance(raw_features, list):
        return []
    return sorted(
...
def _region_result(**kwargs) -> dict[str, object]:
    ...
    if survivors:
        result["status"] = "spatially_supported_steps_inside_recent_released_units"
    elif clusters:
        result["status"] = "clusters_rejected_without_single_recent_released_unit"
    ...
    constraint.update(
        {
            "source_layer": FDEP_RELEASED_UNITS_LAYER_URL,
            "source_description": (
                "FDEP Mandatory Released Phosphate - Reclamation Units"
            ),
            "release_year_min": RELEASE_YEAR_MIN,
            "release_year_max": RELEASE_YEAR_MAX,
            "official_feature_count": len(active_units.get("features", [])),
            "official_unit_identities": _recent_unit_labels(active_units),
    ...
    interpretation.update(
        {
            "recent_released_unit_constraint_applied_before_scanning": True,
            "single_named_unit_gate_applied_after_clustering": True,
            "release_year_is_not_construction_date": True,
...
def _summary(**kwargs) -> dict[str, object]:
    ...
    if isinstance(priorities, list):
        for candidate in priorities:
            ...
            if match is not None and isinstance(
                match.get("official_released_unit"), dict
            ):
                candidate["official_released_unit"] = dict(
                    match["official_released_unit"]
                )

    failed_tiles = int(summary.get("failed_tile_count", 0) or 0)
    candidate_count = int(summary.get("surviving_candidate_count", 0) or 0)
    summary["status"] = (
        "recent_released_unit_candidates_found"
        if candidate_count
        else (
            "recent_released_unit_scan_incomplete"
            if failed_tiles
            else "no_surviving_candidates_in_recent_released_units"
        )
    )
    summary["release_year_min"] = RELEASE_YEAR_MIN
    summary["release_year_max"] = RELEASE_YEAR_MAX
    ...
    if isinstance(region_summaries, list) and region_summaries:
        ...
        if isinstance(first, dict):
            ...
            first["official_polygon_geojson"] = str(
                Path(summary["output_directory"]) / REGION_ID / UNIT_GEOJSON_FILENAME
            )
    ...
    interpretation.update(
        {
            "every_retained_segment_inside_recent_released_unit_polygon": True,
...
def install_campaign() -> None:
    ...
    campaign.FDEP_LAYER_URL = FDEP_RELEASED_UNITS_LAYER_URL
    ...
    campaign.fetch_active_mines = fetch_recent_released_units
    campaign._mine_names = _recent_unit_labels
