...
def test_roi_preview_route_returns_safe_metadata_and_starts_no_run(monkeypatch) -> None:
    with TemporaryDirectory() as temp_dir:
        ...
        with TestClient(create_app(settings), raise_server_exceptions=False) as client:
            response = client.post("/roi/preview", json={"lat": 35.59499, "lon": 36.12694})

        assert response.status_code == 200
        payload = response.json()
        assert payload["mode"] == "point"
        assert payload["selected_point_preview"]["north_south_degrees"] == 35.59499
        assert payload["selected_point_preview"]["east_west_degrees"] == 36.12694
        assert payload["grid_preview"]["width_cells"] == 640
        assert payload["grid_preview"]["height_cells"] == 640
        assert payload["grid_preview"]["cell_size_meters"] == 10
        assert payload["grid_preview"]["reference_system_label"].startswith("EPSG:")
        assert payload["warnings"]
        assert asyncio.run(_run_count(settings)) == 0
        _assert_no_forbidden_public_preview_fields(response.text)
...
def test_invalid_latitude_is_rejected() -> None:
    with TemporaryDirectory() as temp_dir:
        ...
        with TestClient(create_app(settings), raise_server_exceptions=False) as client:
            response = client.post("/roi/preview", json={"lat": 91.0, "lon": 36.12694})
...
def test_invalid_longitude_is_rejected() -> None:
    with TemporaryDirectory() as temp_dir:
        ...
        with TestClient(create_app(settings), raise_server_exceptions=False) as client:
            response = client.post("/roi/preview", json={"lat": 35.59499, "lon": 181.0})
...
def test_nonnumeric_coordinate_is_rejected() -> None:
    with TemporaryDirectory() as temp_dir:
        settings = _settings(Path(temp_dir))
        _upgrade_database(settings)

        with TestClient(create_app(settings), raise_server_exceptions=False) as client:
            response = client.post("/roi/preview", json={"lat": "north", "lon": 36.12694})
...
def test_roi_preview_route_creates_no_artifacts_or_run_directory() -> None:
    with TemporaryDirectory() as temp_dir:
        settings = _settings(Path(temp_dir))
        _upgrade_database(settings)
...
def _raise_if_run_starts(*args, **kwargs) -> None:
    raise AssertionError("ROI preview must not start a backend run.")
...
def _assert_no_forbidden_public_preview_fields(text: str) -> None:
