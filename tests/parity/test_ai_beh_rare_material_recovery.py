...
from app.pipeline.parity.ai_beh_rare_material_recovery import (
    ALLOWED_IMPLEMENTATION_STATUSES,
    ALLOWED_SOURCE_STATUSES,
    AIBehRareMaterialRecoveryItem,
    get_ai_beh_rare_material_recovery_checklist,
    write_ai_beh_rare_material_recovery_report,
)
...
def test_recovery_checklist_covers_all_ai_beh_rare_material_outputs() -> None:
    checklist = get_ai_beh_rare_material_recovery_checklist()
...
def test_recovery_report_writes_and_parses(tmp_path: Path) -> None:
    ...
    report_path = write_ai_beh_rare_material_recovery_report(run_dir, "run-4h9")

    payload = json.loads(report_path.read_text(encoding="utf-8"))

    assert report_path == run_dir / "manifests" / "ai_beh_rare_material_recovery_report.json"
    assert payload["run_id"] == "run-4h9"
    assert payload["phase_4h9_formula_changes"] is False
...
def test_recovery_report_does_not_create_tif_or_npy(tmp_path: Path) -> None:
    run_dir = tmp_path / "run"
    write_ai_beh_rare_material_recovery_report(run_dir, "run-4h9")
