...
def test_verifier_requires_all_three_output_names() -> None:
    assert AI_BEH_RELATION_OUTPUT_NAMES == (
...
def test_verifier_reports_missing_app_output(tmp_path: Path) -> None:
    ...
    for name in AI_BEH_RELATION_OUTPUT_NAMES:
        _write_bytes(ref_dir / name, b"ref")

    result = verify_ai_beh_relation_parity(app_dir, ref_dir, run_dir, "run-4h5")
...
def test_verifier_reports_value_mismatch(tmp_path: Path) -> None:
    ...
    for name in AI_BEH_RELATION_OUTPUT_NAMES:
        _write_tif(app_dir / name, [[1.0, 2.0], [3.0, 4.0]])
        _write_tif(ref_dir / name, [[1.0, 2.0], [3.0, 4.0]])
    _write_tif(ref_dir / AI_BEH_RELATION_OUTPUT_NAMES[1], [[9.0, 9.0], [9.0, 9.0]])

    result = verify_ai_beh_relation_parity(app_dir, ref_dir, run_dir, "run-4h5")

    assert result.overall_status == "failed"
    statuses = {item["output_name"]: item["status"] for item in result.outputs}
    assert statuses["AI_BEH_IronOxide_REL_Ratio_DOM_lin_640.tif"] == "value_mismatch"
...
def test_verifier_records_hashes_and_writes_json_report(tmp_path: Path) -> None:
    ...
    assert result.report_path == run_dir / "manifests" / "ai_beh_relation_parity_verification.json"
...
def test_verifier_does_not_create_tif_or_npy_under_run_dir(tmp_path: Path) -> None:
    ...
    verify_ai_beh_relation_parity(app_dir, ref_dir, run_dir, "run-4h5")
...
def test_docs_and_code_avoid_forbidden_confirmation_wording() -> None:
    doc_text = Path("docs/AI_BEH_RELATION_PARITY_CONTRACT.md").read_text(encoding="utf-8").lower()
    code_text = Path("app/pipeline/parity/ai_beh_relation_recovery.py").read_text(
        encoding="utf-8"
    ).lower()
