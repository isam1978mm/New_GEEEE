...
from app.pipeline.parity.ai_ready_anomaly_verify import (
    AI_READY_ANOMALY_CLASSIFICATION,
    AI_READY_ANOMALY_OUTPUT_NAMES,
    AI_READY_ANOMALY_VERIFICATION_SCHEMA_VERSION,
    verify_ai_ready_anomaly_parity,
)
...
def _write_raster(
    ...
    *,
    ...
    if not RASTERIO_AVAILABLE:
        path.write_bytes(b"placeholder-ai-ready-anomaly-raster")
        return
...
def _write_output_set(directory, *, value=1.0, width=2, height=2):
    for index, name in enumerate(AI_READY_ANOMALY_OUTPUT_NAMES):
        _write_raster(
            directory / name,
...
def test_recovery_checklist_names_are_required():
    assert AI_READY_ANOMALY_OUTPUT_NAMES == (
...
def test_missing_app_output_is_reported_clearly(tmp_path):
    ...
    result = verify_ai_ready_anomaly_parity(
    ...
    assert all(
        item["notebook_value_parity_verified"] is False for item in report["outputs"]
    )
...
def test_tif_comparison_passes_when_rasterio_is_available(tmp_path):
    ...
    result = verify_ai_ready_anomaly_parity(app_dir, reference_dir, run_dir, "run-pass")
    report = _load_report(result.report_path)

    assert report["schema_version"] == AI_READY_ANOMALY_VERIFICATION_SCHEMA_VERSION
    ...
    assert all(item["runtime_output_verified"] is True for item in report["outputs"])
    assert all(
        item["notebook_value_parity_verified"] is True for item in report["outputs"]
    )
...
def test_hashes_report_json_and_run_dir_scope_are_recorded(tmp_path):
    ...
    result = verify_ai_ready_anomaly_parity(app_dir, reference_dir, run_dir, "run-report")
    report = _load_report(result.report_path)

    assert result.report_path == run_dir / "manifests" / "ai_ready_anomaly_parity_verification.json"
...
def test_verification_does_not_create_tif_or_npy_files_under_run_dir(tmp_path):
    ...
    verify_ai_ready_anomaly_parity(app_dir, reference_dir, run_dir, "run-no-writes")
    created = [
        path
...
def test_classification_exposure_and_runtime_flags_remain_private(tmp_path):
    ...
    assert report["classification"] == AI_READY_ANOMALY_CLASSIFICATION
    assert report["target_mode"] == "notebook_parity"
    assert report["target_mode"] != "public_shared"
    assert report["http_servable"] is False
    assert all(item["classification"] == AI_READY_ANOMALY_CLASSIFICATION for item in report["outputs"])
...
def test_phase_4k_module_does_not_add_compute_generate_alias_copy_or_semantic_math_functions():
    import app.pipeline.parity.ai_ready_anomaly_verify as verifier
...
def test_no_forbidden_confirmation_wording_is_introduced_in_doc_or_code_comments():
    doc_text = Path("docs/AI_READY_ANOMALY_PARITY_CONTRACT.md").read_text(
        encoding="utf-8"
    ).lower()
    verify_text = Path("app/pipeline/parity/ai_ready_anomaly_verify.py").read_text(
        encoding="utf-8"
    ).lower()
    recovery_text = Path("app/pipeline/parity/ai_ready_anomaly_recovery.py").read_text(
        encoding="utf-8"
    ).lower()
