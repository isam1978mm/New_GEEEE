...
from app.pipeline.parity.sar_asc_desc_verify import (
    SAR_ASC_DESC_OUTPUT_NAMES,
    SAR_ASC_DESC_VERIFICATION_SCHEMA_VERSION,
...
def _write_tif(path, values, *, width=2, height=2, crs="EPSG:32636", transform=None, dtype="float32", nodata=-9999.0):
    ...
    if not RASTERIO_AVAILABLE:
        path.write_bytes(b"placeholder-sar-support-raster")
        return
...
def _write_all_matching_outputs(app_dir, reference_dir):
    for name in SAR_ASC_DESC_OUTPUT_NAMES:
...
def test_all_eight_output_names_are_required():
    assert SAR_ASC_DESC_OUTPUT_NAMES == (
...
def test_missing_app_output_is_reported_clearly(tmp_path):
    ...
    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-missing-app")
...
def test_missing_reference_output_is_reported_clearly(tmp_path):
    ...
    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-missing-reference")
    report = _load_report(result.report_path)
...
def test_npy_matching_fixtures_pass_and_hashes_are_recorded(tmp_path):
    ...
    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-npy-pass")
    report = _load_report(result.report_path)
    npy_outputs = [item for item in report["outputs"] if item["file_type"] == "npy"]

    assert report["schema_version"] == SAR_ASC_DESC_VERIFICATION_SCHEMA_VERSION
    assert len(npy_outputs) == 4
...
def test_npy_shape_mismatch_fails(tmp_path):
    ...
    reference_dir = tmp_path / "reference"
    _write_all_matching_outputs(app_dir, reference_dir)
    _write_npy(app_dir / "S1_ASC_VV_Filtered_640.npy", [[1.0, 2.0]])

    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-shape")
    report = _load_report(result.report_path)
    item = _output_by_name(report, "S1_ASC_VV_Filtered_640.npy")
...
def test_npy_dtype_mismatch_fails(tmp_path):
    ...
    reference_dir = tmp_path / "reference"
    _write_all_matching_outputs(app_dir, reference_dir)
    _write_npy(app_dir / "S1_ASC_VH_Filtered_640.npy", [[1, 2], [3, 4]], dtype=np.int16)

    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-dtype")
    report = _load_report(result.report_path)
    item = _output_by_name(report, "S1_ASC_VH_Filtered_640.npy")
...
def test_npy_value_mismatch_fails(tmp_path):
    ...
    reference_dir = tmp_path / "reference"
    _write_all_matching_outputs(app_dir, reference_dir)
    _write_npy(app_dir / "S1_DESC_VV_Filtered_640.npy", [[100.0, 2.0], [3.0, 4.0]])

    result = verify_sar_asc_desc_support_stack_parity(
        app_dir,
        reference_dir,
        run_dir,
        "run-value",
    ...
    report = _load_report(result.report_path)
    item = _output_by_name(report, "S1_DESC_VV_Filtered_640.npy")
...
def test_tif_comparison_passes_when_rasterio_is_available(tmp_path):
    ...
    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-tif-pass")
    report = _load_report(result.report_path)
...
def test_tif_value_mismatch_fails_when_rasterio_is_available(tmp_path):
    ...
    reference_dir = tmp_path / "reference"
    _write_all_matching_outputs(app_dir, reference_dir)
    _write_tif(app_dir / "S1_DESC_VH_Filtered_640.tif", np.full((2, 2), 9.0, dtype=np.float32))

    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-tif-value")
    report = _load_report(result.report_path)
    item = _output_by_name(report, "S1_DESC_VH_Filtered_640.tif")
...
def test_tif_comparison_unavailable_when_rasterio_is_not_available(tmp_path):
    ...
    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-no-rasterio")
    report = _load_report(result.report_path)
    tif_outputs = [item for item in report["outputs"] if item["file_type"] == "tif"]
...
def test_report_json_writes_parses_and_stays_under_run_dir(tmp_path):
    ...
    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-report")
    report = _load_report(result.report_path)

    assert result.report_path == run_dir / "manifests" / "sar_asc_desc_support_stack_verification.json"
...
def test_verification_does_not_create_raster_npy_or_alias_files_under_run_dir(tmp_path):
    ...
    verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-no-output-writes")
    created = [
        path
    ...
    assert created == []
    assert not (run_dir / "GEOTIFF_RADAR_BANDS").exists()
    assert not (run_dir / "NPY_RADAR_BANDS").exists()
...
def test_classification_and_exposure_remain_private_notebook_parity(tmp_path):
    ...
    result = verify_sar_asc_desc_support_stack_parity(app_dir, reference_dir, run_dir, "run-classification")
    report = _load_report(result.report_path)
...
def test_phase_4e2_module_does_not_add_generation_alias_copy_or_sar_math_functions():
    import app.pipeline.parity.sar_asc_desc_verify as verifier
