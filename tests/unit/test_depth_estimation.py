...
def _write_run_quality(run_dir: Path, *, status: str = "PASS", is_usable: bool = True) -> None:
    _write_json(
        ...
        {"schema": "run_quality_summary_v1", "stage": "run_quality", "status": status, "is_usable": is_usable},
    )


def _write_candidates(run_dir: Path, candidates: list[dict[str, str]]) -> None:
    _write_json(run_dir / "depth_inputs" / "candidates.json", {"schema_version": "local_depth_candidates_v1", "candidates": candidates})
...
def _write_package(package_dir: Path) -> Path:
    ...
    manifest = {
        ...
        "zones": [
            {"zone_id": "tyrone_tp5", "depth_min_m": 0.65532, "depth_best_m": 0.68072, "depth_max_m": 0.70612, "warnings": ["measured_anchor"]},
            {"zone_id": "tyrone_tp6", "depth_min_m": 0.8509, "depth_best_m": 0.94996, "depth_max_m": 1.04902, "warnings": ["measured_anchor"]},
        ],
    }
    ...
    (package_dir / "checksums.sha256").write_text(f"{digest}  depth_method_manifest.json\n", encoding="utf-8")
    return package_dir
...
def test_depth_stage_is_disabled_without_outputs_by_default(tmp_path: Path) -> None:
    ...
    run_dir.mkdir()
    settings = Settings(data_dir=tmp_path, database_path=tmp_path / "app.db", local_depth_mode="off")
    result = asyncio.run(DepthEstimationStage().run(StageContext(run_id="run-1", settings=settings, run_dir=run_dir)))
...
def test_local_supported_zone_writes_provisional_metre_range(tmp_path: Path) -> None:
    ...
    _write_candidates(run_dir, [{"candidate_id": "candidate-1", "zone_id": "tyrone_tp5"}])
...
def test_candidate_outside_package_support_has_no_metre_values(tmp_path: Path) -> None:
    ...
    package_dir = _write_package(tmp_path / "package")
    _write_run_quality(run_dir)
    _write_candidates(run_dir, [{"candidate_id": "candidate-2", "zone_id": "unknown-zone"}])
    paths, summary = write_depth_outputs(run_dir=run_dir, package_dir=package_dir)
    row = _read_first_csv_row(paths.estimates_csv)
    ...
    assert "no_extrapolation_outside_reviewed_zones" in row["warnings"]
...
def test_blocked_run_quality_has_no_metre_values(tmp_path: Path) -> None:
    ...
    _write_candidates(run_dir, [{"candidate_id": "candidate-3", "zone_id": "tyrone_tp6"}])
    paths, summary = write_depth_outputs(run_dir=run_dir, package_dir=package_dir)
    row = _read_first_csv_row(paths.estimates_csv)
...
def test_enabled_stage_outputs_private_non_http_artifacts(tmp_path: Path) -> None:
    ...
    package_dir = _write_package(tmp_path / "package")
    _write_run_quality(run_dir)
    _write_candidates(run_dir, [{"candidate_id": "candidate-4", "zone_id": "tyrone_tp6"}])
    settings = Settings(data_dir=tmp_path, database_path=tmp_path / "app.db", local_depth_mode="local_calibrated", local_depth_package_dir=package_dir)
    result = asyncio.run(DepthEstimationStage().run(StageContext(run_id="run-2", settings=settings, run_dir=run_dir)))
