...
def test_classifier_uses_valid_mask_for_patch_features(tmp_path: Path) -> None:
    ...
    assert float(classifications[0]["class_score"]) < 0.2
    assert classifications[0]["valid_pixel_count"] == 1
    assert classifications[0]["valid_pixel_fraction"] == 0.0625
    assert classifications[0]["feature_policy"] == "private_local_robust_object_evidence_v1"
...
def test_classifier_preserves_negative_feature_values_before_scoring(tmp_path: Path) -> None:
    ...
    assert classifications[0]["signal_mean_raw"] == -0.75
    assert classifications[0]["signal_mean_z"] < 0.0
    assert classifications[0]["anomaly_peak"] == 0.0
    assert classifications[0]["valid_pixel_fraction"] == 1.0
    assert classifications[0]["class_score"] < 0.4


def test_classifier_outputs_private_local_object_evidence_columns(tmp_path: Path) -> None:
    run_dir = tmp_path / "run"
    run_dir.mkdir()
    grid_spec = build_run_grid(0.0, 0.0)
    hypercube = np.zeros((grid_spec.size, grid_spec.size, 3), dtype=np.float32)
    hypercube[:, :, 0:2] = 0.0
    hypercube[:, :, 2] = 1.0
    hypercube[20:24, 30:34, 0:2] = 3.0
    np.save(run_dir / HYPERCUBE_NPY_NAME, hypercube)

    anomaly = np.zeros((grid_spec.size, grid_spec.size), dtype=np.float32)
    anomaly[20:24, 30:34] = 0.8
    Image.fromarray(anomaly).save(run_dir / PCA_ANOMALY_TIF_NAME, format="TIFF")
    _write_csv(
        run_dir / OBJECTS_INDEX_NAME,
        ["object_id", "cluster_id", "row_min", "row_max", "col_min", "col_max", "area_px"],
        [{"object_id": 1, "cluster_id": 0, "row_min": 20, "row_max": 23, "col_min": 30, "col_max": 33, "area_px": 12}],
    )
    _write_csv(
        run_dir / CLUSTERS_SUMMARY_NAME,
        ["cluster_id", "object_count", "total_area_px", "mean_object_area_px", "max_object_anomaly"],
        [{"cluster_id": 0, "object_count": 1, "total_area_px": 12, "mean_object_area_px": 12.0, "max_object_anomaly": 0.8}],
    )

    classifications, summary = build_classifier_results(run_dir)

    row = classifications[0]
    assert summary["classifier_feature_policy"] == "private_local_robust_object_evidence_v1"
    assert row["signal_mean_raw"] == 3.0
    assert row["signal_mean_z"] > 0.0
    assert row["anomaly_peak"] == 0.8
    assert row["object_area_px"] == 12
    assert row["bbox_area_px"] == 16
    assert row["compactness"] == 0.75
    assert row["valid_pixel_fraction"] == 1.0
    assert row["feature_policy"] == "private_local_robust_object_evidence_v1"
