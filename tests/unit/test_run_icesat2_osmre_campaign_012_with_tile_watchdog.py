...
MODULE_PATH = (
    ...
    / "scripts"
    / "run_icesat2_osmre_campaign_012_with_tile_watchdog.py"
)
SPEC = importlib.util.spec_from_file_location("campaign012_watchdog", MODULE_PATH)
...
def test_watchdog_converts_subprocess_timeout_to_tile_failure(monkeypatch):
    ...
    with pytest.raises(MODULE.Campaign012TileTimeoutError, match="300 seconds"):
...
def test_install_timeout_hook_changes_only_query_hook():
    original = MODULE.campaign012.campaign._query_atl08
    original_region_result = MODULE.campaign012.campaign._region_result
    original_summary = MODULE.campaign012.campaign._summary
    try:
        MODULE.install_timeout_hook()
        assert MODULE.campaign012.campaign._query_atl08 is MODULE._query_atl08_with_timeout
        assert MODULE.campaign012.campaign._region_result is original_region_result
        assert MODULE.campaign012.campaign._summary is original_summary
    finally:
        MODULE.campaign012.campaign._query_atl08 = original
